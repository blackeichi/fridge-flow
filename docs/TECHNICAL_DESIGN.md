...
후기 결정이 같은 주제의 이전 결정을 대체한다.

### 현재 결정

| 날짜 | 결정 | 근거 |
|---|---|---|
| 2026-07-29 | Expo React Native Android 앱 | 개인·지인용 Android 배포와 빠른 검증 |
| 2026-07-29 | Drizzle ORM + `useLiveQuery` | SQLite typed schema·migration·반응형 조회 일관성 |
| 2026-07-29 | Responses API + 구조화 출력 | 검증 가능한 AI 출력 계약 |
| 2026-07-29 | 초기 `OPENAI_MODEL=gpt-5.4-mini` | 초기 API 비용을 낮추면서 레시피·식단 생성 품질 유지 |
| 2026-07-29 | Firebase App Distribution + EAS Build APK | 지인 대상 공식 build 전달과 재현 가능한 cloud build |
| 2026-07-29 | `.ffbackup` 비암호화 | 개인용 복원 편의성을 우선하고 checksum으로 손상 검증 |
| 2026-08-02 | `user.db` 로컬 단일 원본 | 중앙 DB·동기화 복잡도를 제거하고 모든 핵심 기능의 offline 동작 보장 |
| 2026-08-02 | Android SAF 기반 Google Drive 백업 | Drive token·서버 파일 저장 없이 사용자 선택 위치로 직접 export/import |
| 2026-08-02 | Node.js TypeScript Netlify Functions | 사용자 DB 없이 Google auth와 OpenAI 호출만 담당하는 stateless gateway |
| 2026-08-02 | Google ID token + 환경변수 allowlist | 서버 DB 없이 허용된 Google 계정만 AI 비용을 발생시킬 수 있게 제한 |
| 2026-08-02 | 환경변수 모델 강제 | 변조된 client가 고가 모델이나 token 상한을 선택하지 못하게 함 |
| 2026-08-02 | Netlify IP rate limit + OpenAI project limits | durable quota store 없이 가능한 stateless 비용 방어를 적용하고 한계를 명시 |
| 2026-08-02 | 사용자 주도 Drive 내보내기 | MVP에서 자동 백업·동기화보다 예측 가능한 수동 복구 경로 우선 |

### 폐기된 서버 구조

| 날짜 | 폐기 결정 | 대체 |
|---|---|---|
| 2026-08-02 | Python 3.13 / FastAPI / Uvicorn | Node.js 22 TypeScript Netlify Functions |
| 2026-08-02 | Render Free Web Service | Netlify Functions Git deploy |
| 2026-08-02 | Neon PostgreSQL 서버 원본 | Android `user.db` 로컬 단일 원본 |
| 2026-08-02 | AppUser/AppInvite/RLS 서버 계정 | Google token 검증 + 환경변수 AI allowlist |
| 2026-08-02 | outbox/cursor 양방향 동기화 | 중앙 동기화 없음; `.ffbackup` 전체 교체 복원 |
| 2026-08-02 | 서버 restore transaction | 임시 SQLite 검증 후 로컬 DB 원자 교체 |
| 2026-08-02 | Neon quota transaction·lease | Netlify IP rate limit과 OpenAI project limits; 누적 quota 한계 수용 |
| 2026-08-02 | Sentry FastAPI 오류 추적 | Netlify 기본 logs/metrics로 시작하고 필요 시 재검토 |
