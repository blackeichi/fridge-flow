...
| 기기 분실·앱 삭제 | 마지막 Drive 내보내기 이후 데이터 손실 | 최근 백업 시각 표시, 7일 경고, 위험 작업 전 안전 백업 |
...
| AI endpoint 무단 호출 | 비용 발생 또는 정상 사용자의 한도 소진 | Google token 검증, 환경변수 허용 목록, IP rate limit, 고정 모델·token 한도와 kill switch |
| stateless 함수의 누적 quota 부재 | 사용자별 일·월 hard cap을 정확히 강제하지 못함 | 소수 허용 계정, OpenAI 프로젝트 model/rate 설정과 사용량 알림; 필요 시에만 영속 quota 저장소 도입 |
| 다중 기기 수정 | 서로 다른 로컬 데이터가 자동 병합되지 않음 | MVP는 단일 주 기기 권장, 최신 Drive 백업을 전체 교체 복원하고 병합은 제공하지 않음 |
| 초대 APK 재공유 | 허용하지 않은 사람이 앱 설치 | Firebase 배포 대상 제한; AI 비용 경계는 서버 Google 계정 허용 목록으로 별도 보호 |
...
같은 주제의 후기 결정이 이전 결정을 대체한다. `폐기`로 표시된 항목은 현재 설계가 아니라 변경 이력이다.
...
| 2026-07-29 | 앱 시작부터 Google 로그인 필수 재채택 | 현재는 로컬 데이터 소유 계정 연결과 AI 접근 검증 목적으로 유지 |
...
| 2026-07-29 | Neon PostgreSQL 계정별 서버 원본 — 폐기 | 2026-08-02 로컬 SQLite 단일 원본으로 대체 |
| 2026-07-29 | 로컬 SQLite replica + outbox/cursor 동기화 — 폐기 | 2026-08-02 local-only 구조로 대체 |
| 2026-07-29 | Google 이메일 초대 목록 — 폐기 | DB 초대 대신 Netlify 환경변수의 AI 허용 계정 목록으로 대체 |
| 2026-07-29 | 초대와 계정 상태 분리 — 폐기 | 서버 계정 자체를 운영하지 않음 |
| 2026-07-29 | 로그아웃 시 로컬 계정 데이터 삭제 — 폐기 | 서버 복구가 없으므로 로그아웃과 기기 데이터 초기화를 분리 |
| 2026-07-29 | 기기당 단일 계정 replica | 로컬 DB 구조와 로그아웃·계정 전환을 단순하고 안전하게 유지 |
| 2026-07-29 | 회원 탈퇴 즉시 전체 삭제 — 폐기 | 서버 계정이 없어 `기기 데이터 초기화`로 대체 |
| 2026-07-29 | Firebase App Distribution 초대 배포 | 공식 APK 전달 대상을 제한하되 AI 접근 보안 경계로 간주하지 않음 |
| 2026-07-29 | FastAPI + Render API — 폐기 | 2026-08-02 Node.js Netlify AI 전용 백엔드로 대체 |
| 2026-07-29 | Render 무료 서버 cold start UX — 폐기 | Render 서비스를 사용하지 않음 |
| 2026-07-29 | Render·Neon Singapore 리전 — 폐기 | Render와 Neon을 사용하지 않음 |
| 2026-07-29 | 인증 + AI 사용량 상한 | 계정별 접근을 검증하고 별도의 비용 상한으로 키 유출·오남용 피해 제한 |
| 2026-08-02 | 로컬 SQLite 단일 원본으로 변경 | 중앙 DB·동기화 복잡도를 제거하고 개인·지인용 앱의 오프라인 우선 요구에 집중 |
| 2026-08-02 | Google Drive 사용자 주도 백업 | Android 시스템 파일 선택기로 사용자가 소유한 Drive 위치에 백업하고 서버 저장소를 운영하지 않음 |
| 2026-08-02 | Node.js Netlify AI 전용 백엔드 | 사용자 CRUD 없이 Google token 검증·허용 계정 확인·OpenAI 호출만 수행 |
| 2026-08-02 | 모델은 서버 환경변수로만 선택 | 변조된 앱이 고가 모델을 지정하지 못하게 하고 배포 설정으로 모델을 통제 |
| 2026-08-02 | FastAPI·Render·Neon·서버 동기화 폐기 | 새 local-only + stateless AI gateway 구조와 중복되는 서버·DB 운영 요소 제거 |
